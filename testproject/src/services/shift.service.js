...
const getMasterData = type => {
  return axiosInstance
    .get(`${API_PATH.API_VERSION}${API_PATH.MASTER_DATA}${type}`)
    .then(response => {
      return response;
    });
};
...
const createShift = data => {
  return axiosInstance
    .post(`${API_PATH.API_VERSION}${API_PATH.CREATE_SHIFT}`, data)
    .then(response => {
      return response;
    });
};
...
const updateShift = data => {
  return axiosInstance
    .put(`${API_PATH.API_VERSION}${API_PATH.SHIFT_UPDATE}`, data)
    .then(response => {
      return response;
    });
};

const getAllShifts = (id, type, shiftType) => {
  let url = `${API_PATH.API_VERSION}${API_PATH.SHIFT}/${id}/${type}/${shiftType}`;
  //   if (queryParams) {
  //     url += `/${queryParams}`;
  //   }
...
const shiftTeamZoneMapping = data => {
  return axiosInstance
    .post(
      `${API_PATH.API_VERSION}${API_PATH.CREATE_SHIFT}${API_PATH.SHIFT_TEAM_MAPPING}`,
      data,
    )
    .then(response => {
      return response;
